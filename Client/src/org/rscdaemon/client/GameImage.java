...
public class GameImage implements ImageProducer, ImageObserver {
  ...
  public synchronized void completePixels() {
    ...
    imageConsumer.setPixels(0, 0, menuDefaultWidth, menuDefaultHeight, colourModel, imagePixelArray, 0,
        menuDefaultWidth);
  ...
  public void method214(int i, int j, int k, int l, int i1, int j1) {
    ...
    for (int k3 = 0; k3 < l; k3 += byte0)
      if (k3 + j >= imageY && k3 + j < imageHeight) {
        int l3 = ((k1 * k3 + j2 * (l - k3)) / l << 16) + ((l1 * k3 + k2 * (l - k3)) / l << 8)
            + (i2 * k3 + l2 * (l - k3)) / l;
        for (int i4 = -k; i4 < 0; i4++)
  ...
  private void plotScale2(int ai[], int ai1[], int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2,
      int j2, int k2, int l2) {
    int i3 = l2 >> 16 & 0xff;
    int j3 = l2 >> 8 & 0xff;
    int k3 = l2 & 0xff;
  ...
  public void spriteClip4(int i, int j, int k, int l, int i1, int overlay, int k1, int l1, boolean flag) {
    try {
      ...
      if (k1 == 0xffffff) {
        if (!flag) {
          spritePlotTransparent(imagePixelArray, sprites[i1].getPixels(), 0, k2, l2, j4, k, l, j3, k3, i2, overlay, i3,
              l3, i5);
          return;
        } else {
          spritePlotTransparent(imagePixelArray, sprites[i1].getPixels(), 0, (sprites[i1].getWidth() << 16) - k2 - 1,
              l2, j4, k, l, -j3, k3, i2, overlay, i3, l3, i5);
          return;
        }
      }
      if (!flag) {
        spritePlotTransparent(imagePixelArray, sprites[i1].getPixels(), 0, k2, l2, j4, k, l, j3, k3, i2, overlay, k1,
            i3, l3, i5);
        return;
      } else {
        spritePlotTransparent(imagePixelArray, sprites[i1].getPixels(), 0, (sprites[i1].getWidth() << 16) - k2 - 1, l2,
            j4, k, l, -j3, k3, i2, overlay, k1, i3, l3, i5);
        return;
      }
    }
    catch (Exception _ex) {
  ...
  private void spritePlotTransparent(int ai[], int ai1[], int i, int j, int k, int l, int i1, int j1, int k1, int l1,
      int i2, int overlay, int k2, int l2, int i3) {
  ...
  private void spritePlotTransparent(int ai[], int ai1[], int i, int j, int k, int l, int i1, int j1, int k1, int l1,
      int i2, int overlay, int k2, int l2, int i3, int j3) {
  ...
  private void spritePlotTransparent(int ai[], byte abyte0[], int ai1[], int i, int j, int k, int l, int i1, int j1,
      int k1, int l1, int i2, int overlay, int k2, int l2, int i3) {
    int i4 = overlay >> 16 & 0xff;
    int j4 = overlay >> 8 & 0xff;
    int k4 = overlay & 0xff;
    try {
  ...
  public static void drawLetter(Font font, FontMetrics fontmetrics, char letter, int charSetOffset,
      GameWindow gameWindow, int fontNumber, boolean addCharWidth) {
    ...
    if (addCharWidth)
      try {
        if (letter == '/')
          addCharWidth = false;
        if (letter == 'f' || letter == 't' || letter == 'w' || letter == 'v' || letter == 'k' || letter == 'x'
            || letter == 'y' || letter == 'A' || letter == 'V' || letter == 'W')
          charWidth++;
      }
      catch (Exception _ex) {
      }
      ...
      for (int offset = 0; offset < string.length(); offset++)
        if (string.charAt(offset) == '@' && offset + 4 < string.length() && string.charAt(offset + 4) == '@') {
          if (string.substring(offset + 1, offset + 4).equalsIgnoreCase("red"))
            colour = 0xff0000;
          else if (string.substring(offset + 1, offset + 4).equalsIgnoreCase("adm")) // Yellow
                                                                                     // Crown
            spriteClip4(x - 12, y - 16, 30, 20, 2339, -256, 0, 0, false);
          else if (string.substring(offset + 1, offset + 4).equalsIgnoreCase("mod")) // Grey
                                                                                     // Crown
            spriteClip4(x - 12, y - 16, 30, 20, 2339, -2302756, 0, 0, false);
          else if (string.substring(offset + 1, offset + 4).equalsIgnoreCase("pmd")) // Green
                                                                                     // Crown
            spriteClip4(x - 12, y - 16, 30, 20, 2339, -13382656, 0, 0, false);
          else if (string.substring(offset + 1, offset + 4).equalsIgnoreCase("evt")) // Blue
                                                                                     // Crown
            spriteClip4(x - 12, y - 16, 30, 20, 2339, -10245124, 0, 0, false);
          else if (string.substring(offset + 1, offset + 4).equalsIgnoreCase("rad")) // Red
                                                                                     // Crown
            spriteClip4(x - 12, y - 16, 30, 20, 2339, 13369344, 0, 0, false);
          else if (string.substring(offset + 1, offset + 4).equalsIgnoreCase("blk")) // Black
                                                                                     // Crown
            spriteClip4(x - 12, y - 16, 30, 20, 2339, 2500134, 0, 0, false);
          else if (string.substring(offset + 1, offset + 4).equalsIgnoreCase("pnk")) // Pink
                                                                                     // Crown
            spriteClip4(x - 12, y - 16, 30, 20, 2339, 16711935, 0, 0, false);
          ...
          offset += 4;
            ...
            && string.substring(offset + 1, offset + 4).equalsIgnoreCase("adm")) { // Yellow
                                                                                   // Crown
          spriteClip4(x - 12, y - 16, 30, 20, 2339, -256, 0, 0, false);
          x += 14;
          offset += 4;
        } else if (string.charAt(offset) == '#' && offset + 4 < string.length() && string.charAt(offset + 4) == '#'
            && string.substring(offset + 1, offset + 4).equalsIgnoreCase("mod")) { // Grey
                                                                                   // Crown
          spriteClip4(x - 12, y - 16, 30, 20, 2339, -2302756, 0, 0, false);
          x += 14;
          offset += 4;
        } else if (string.charAt(offset) == '#' && offset + 4 < string.length() && string.charAt(offset + 4) == '#'
            && string.substring(offset + 1, offset + 4).equalsIgnoreCase("pmd")) { // Green
                                                                                   // Crown
          spriteClip4(x - 12, y - 16, 30, 20, 2339, -13382656, 0, 0, false);
          x += 14;
          offset += 4;
        } else if (string.charAt(offset) == '#' && offset + 4 < string.length() && string.charAt(offset + 4) == '#'
            && string.substring(offset + 1, offset + 4).equalsIgnoreCase("evt")) { // Blue
                                                                                   // Crown
          spriteClip4(x - 12, y - 16, 30, 20, 2339, -10245124, 0, 0, false);
          x += 14;
          offset += 4;
        } else if ((string.charAt(offset) == '#') && (offset + 4 < string.length())
            && (string.charAt(offset + 4) == '#')
            && (string.substring(offset + 1, offset + 4).equalsIgnoreCase("red"))) { // Red
                                                                                     // Crown
          spriteClip4(x - 12, y - 16, 30, 20, 2339, 13369344, 0, 0, false);
          x += 14;
          offset += 4;
            ...
            && (string.charAt(offset + 4) == '#')
            && (string.substring(offset + 1, offset + 4).equalsIgnoreCase("pnk"))) { // Pink
                                                                                     // Crown
          spriteClip4(x - 12, y - 16, 30, 20, 2339, 16711935, 0, 0, false);
          x += 14;
          offset += 4;
            ...
            && (string.charAt(offset + 4) == '#')
            && (string.substring(offset + 1, offset + 4).equalsIgnoreCase("blk"))) { // Black
                                                                                     // Crown
          spriteClip4(x - 12, y - 16, 30, 20, 2339, 2500134, 0, 0, false);
          x += 14;
          offset += 4;
            ...
            && (string.charAt(offset + 4) == '#')
            && (string.substring(offset + 1, offset + 4).equalsIgnoreCase("rin"))) { // Rainbow
                                                                                     // Crown
          colour = (int) (Math.random() * 16777215D);
          spriteClip4(x - 12, y - 16, 30, 20, 2339, colour, 0, 0, false);
          x += 14;
          offset += 4;
        } else if ((string.charAt(offset) == '#') && (offset + 4 < string.length())
            && (string.charAt(offset + 4) == '#')
            && (string.substring(offset + 1, offset + 4).equalsIgnoreCase("orn"))) { // Orange
                                                                                     // Crown
          spriteClip4(x - 12, y - 16, 30, 20, 2339, -2393283, 0, 0, false);
          x += 14;
          offset += 4;
        } else {
