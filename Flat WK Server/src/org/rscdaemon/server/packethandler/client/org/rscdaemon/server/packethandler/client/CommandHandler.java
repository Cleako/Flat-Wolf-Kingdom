...
public class CommandHandler implements PacketHandler {
  ...
  public void handleCommand(String cmd, String[] args, Player player, String raw) throws Exception {
    ...
    if (cmd.equals("wipebank")) {
      ...
      player.getActionSender().sendMessage("@cya@[@red@Wolf Kingdom@cya@] @whi@Your bank has been cleared.");
      return;
    }
    if (cmd.equals("skull")) {
      player.addSkull(1200000);
      player.getActionSender().sendMessage("@cya@[@red@Wolf Kingdom@cya@] @whi@You are now skulled for 20 minutes!");
      return;
    }
    if (cmd.equals("fatigue")) {
      player.setFatigue(100);
      player.getActionSender().sendFatigue();
      player.getActionSender().sendMessage("@cya@[@red@Wolf Kingdom@cya@] @whi@Your fatigue has been set to 100%");
      return;
    }
    if (cmd.equals("spree")) {
      ...
      if (p != null) {
        player.getActionSender()
            .sendMessage("[@red@Wolf Kingdom@whi@] " + p.getUsername() + "'s killing spree is: " + p.getKillingSpree());
      } else {
        player.getActionSender().sendMessage("[@red@Wolf Kingdom@whi@] Invalid player. Please make sure they are online!");
      }
      return;
    }
    if (cmd.equals("wild")) {
      ...
      if (wildcount == 0) {
        player.getActionSender().sendMessage(
            "[@red@Wolf Kingdom@whi@] There is currently @or1@" + wildcount + " @whi@players in the wilderness.");
        return;
      }
      if (wildcount == 1) {
        player.getActionSender().sendMessage(
            "[@red@Wolf Kingdom@whi@] There is currently @or1@" + wildcount + " @whi@player in the wilderness.");
        return;
      } else {
        player.getActionSender().sendMessage(
            "[@red@Wolf Kingdom@whi@] There is currently @or1@" + wildcount + " @whi@players in the wilderness.");
    ...
    if (cmd.equals("say")) {
      ...
      if (player.isMuted()) {
        player.getActionSender().sendMessage("[@red@Wolf Kingdom@whi@] You can't use this command while you're muted!");
        return;
      }
      if (player.isAdmin()) {
    ...
    if (cmd.equals("mute")) {
      ...
      if (args.length != 1) {
        player.getActionSender().sendMessage("[@red@Wolf Kingdom@whi@] Invalid args. Syntax: MUTE name!");
        return;
      }
      if (mute) {
        if (Integer.valueOf(Server.readValue(args[0], "mute")) == 1) {
          player.getActionSender().sendMessage("[@red@Wolf Kingdom@whi@] That player is already @red@muted@whi@!");
          return;
        } else {
          world.mutePlayer(args[0]);
          Player affectedPlayer = world.getPlayer(DataConversions.usernameToHash(args[0]));
          player.getActionSender()
              .sendMessage("[@red@Wolf Kingdom@whi@] You have @red@muted @whi@" + affectedPlayer.getUsername() + "!");
          affectedPlayer.getActionSender()
              .sendMessage("[@red@Wolf Kingdom@whi@] You have been @red@muted @whi@by: " + player.getUsername());
        ...
        if (affectedPlayer == null) {
          player.getActionSender()
              .sendMessage("[@red@Wolf Kingdom@whi@] Invalid player. Please make sure they are online!");
          return;
        }
    ...
    if (cmd.equals("unmute")) {
      ...
      if (unmute) {
        if (Integer.valueOf(Server.readValue(args[0], "mute")) == 0) {
          player.getActionSender().sendMessage("[@red@Wolf Kingdom@whi@] That player is not even muted!");
          return;
        } else {
          if (Integer.valueOf(Server.readValue(args[0], "mute")) == 1) {
            ...
            Player affectedPlayer = world.getPlayer(DataConversions.usernameToHash(args[0]));
            player.getActionSender()
                .sendMessage("[@red@Wolf Kingdom@whi@] You have @gre@unmuted @whi@" + affectedPlayer.getUsername() + "");
            affectedPlayer.getActionSender().sendMessage("[@red@Wolf Kingdom@whi@] You have been @gre@unmuted@whi@!");
            affectedPlayer.getActionSender().sendLogout();
            affectedPlayer.destroy(true);
          }
          Player affectedPlayer = world.getPlayer(DataConversions.usernameToHash(args[0]));
